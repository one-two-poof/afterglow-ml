...
def imsi(imsi_data: RecommendationRequest) -> RecommendationResponse:
    dummy_response = RecommendationResponse(
        status="success",
        message="성공적으로 날짜별 추천 코스를 생성했습니다. (테스트용)",
        data=RecommendationResponseData(
            daily_recommendations=[
                DailyRecommendation(
                    ...
                    treatment=[
                        TreatmentItem(name="비만(약처방)", date="2026-06-08"),
                        TreatmentItem(name="리프팅", date="2026-06-08")
                    ],
                    recommended_courses=[
                        RecommendedCourse(
                            rank=1,
                            course_id="C00003",
                            total_distance_km=4.4,
                            places=[
                                PlaceItem(
                                    visit_order=1,
                                    place_name="매직포레스트",
